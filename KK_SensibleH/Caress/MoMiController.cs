...
namespace KK_SensibleH
{
    ...
    public class MoMiController : MonoBehaviour
    {
        ...
        /// Hook for MainGameVR.
        /// </summary>
        public static void OnLickStart()
        {
            // First call before mouse clicks to stop our side if we are active.
            // Second to do god knows what. But it just works.
            if (!_lickCo)
            {
                if (_moMiCo)
                {
        ...
        /// Hook for MainGameVR.
        /// </summary>
        public static void OnKissStart()
        {
            if (!_kissCo)
            {
                if (_moMiCo)
        ...
        private void Update()
        {
            ...
            // There is little to no point to run moMi on the kiss outside of VR. So we don't, saves ADHD VR users from quite a few bugs.
            if (!_moMiCo && GameCursor.isLock && (_handCtrl.actionUseItem != -1 || _handCtrl.isKiss))
            {
                StartMoMi();
            }
            else if (_moMiCo)
